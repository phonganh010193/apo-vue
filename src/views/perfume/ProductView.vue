  ...
  <div class="perfume-container d-flex flex-row">
    <div class="product-list d-flex flex-column">
      <p class="product-header">
        Trang chủ/
        <span style="color: #2d8356">{{
          categoryId !== "100" ? categoryName : "Sản phẩm"
        }}</span>
      </p>
      <h4 class="product-header-title">
        {{ categoryId !== "100" ? categoryName : "TẤT CẢ SẢN PHẨM" }}
      </h4>
        ...
        <div class="product-item" v-for="item in productList" :key="item.id">
          <div class="product-item-content">
            ...
            <div class="price">
              ...
              <p>
                {{ Number(item.sale_price.split(" ").join("")).toLocaleString() }} VND
              </p>
            </div>
          </div>
        </div>
      ...
      <ul class="product-pagination d-flex flex-row">
        <li class="page-item">
          <Link class="page-link" to="#" @click="changePage(page > 0 ? page - 1 : page)"
            >Trước</Link
          >
        </li>
        <li
          :class="index === page ? 'page-item-active' : 'page-item-children'"
        ...
        <li class="page-item">
          <Link
            class="page-link"
            to="#"
            @click="changePage(page < numberPage - 1 ? page + 1 : page)"
            >Sau</Link
          >
        </li>
      </ul>
